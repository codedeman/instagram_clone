...
class ReelCreationScreen extends StatefulWidget {
  const ReelCreationScreen({Key? key}) : super(key: key);

  @override
  _ReelCreationScreenState createState() => _ReelCreationScreenState();
}
...
class _ReelCreationScreenState extends State<ReelCreationScreen> {
  Uint8List? _file;
                ...
                child: Padding(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      _handleNextButton(
                          selectedImageModels); // Call the method to handle the next action
  ...
  void _handleNextButton(List<AssetEntity> selectedImages) async {
    if (selectedImages.isNotEmpty) {
      // Convert the first selected image to Uint8List
      final file = await selectedImages.first.file;
      if (file != null) {
        final imageBytes = await file.readAsBytes();

        // Navigate to NextScreen with the imageBytes
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => NextScreen(selectedImage: imageBytes),
          ),
        );
      } else {
        // Handle the case where the file is null
        showSnackBar(context, 'Could not load the image');
      }
    }
  }
